...
#include "Render/ssAnimatedModel.h"
...
void Player::Update()
{
	...
	Vector3 ForwardDirection = CameraManager::GetInstance()->GetCameraForwardVector();
	ForwardDirection.Y = 0;
	ForwardDirection.Normalize();
	const Vector3 ForwardMovement = ForwardDirection * (Dir.Y * static_cast<float>(Time::dTimeDelta) * MoveSpeed);

	const Vector3 RightDirection = CameraManager::GetInstance()->GetCameraRightVector();
	...
	CameraManager::GetInstance()->SetCameraPos(NewCamPosition);
}
