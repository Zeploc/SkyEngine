...
bool Button3DEntity::CheckHit(glm::vec3 BottomLeftOffset, glm::vec3 TopRightOffset)
{
	...
	return Utils::CheckFaceHit(BottomLeftOffset,
		TopRightOffset,
		Camera::GetInstance()->GetCameraPosition(),
		Camera::GetInstance()->ScreenToWorldDirection(Input::GetInstance()->MousePos),
		this->shared_from_this(), 
		HitPos);
	
	// glm::vec3 rayDirection = Camera::GetInstance()->ScreenToWorldDirection(Input::GetInstance()->MousePos).ToGLM();
	//
