...
// Static Variables //
TSharedPointer<SoundManager> SoundManager::m_pSoundManager;
...
bool SoundManager::InitFMod()
{
	...
	result = audioMgr->init(50, FMOD_INIT_NORMAL, nullptr);
	if (result != FMOD_OK)
	{
		return false;
	}
...
const bool SoundManager::AddAudio(const char* Path, bool bLoop, std::string sName)
{
	...
	FMOD_RESULT result;
	result = audioMgr->createSound(Path, FMOD_DEFAULT, nullptr, &CreatedSound);
	if (!CreatedSound)
	{
		return false;
	}
...
bool SoundManager::PlayAudio(std::string sName, std::string sChannel)
{
	...
	audioMgr->playSound(GetAudio(sName), nullptr, false, &newChannel);
...
************************************************************/
TSharedPointer<SoundManager> SoundManager::GetInstance()
{
	if (!m_pSoundManager) // null or doesn't exist
	{
		m_pSoundManager = TSharedPointer<SoundManager>(new SoundManager());
	}
